...
def get_bot_response():
    ...
    if "step" in session:
        if session["step"] == "Q_C":
            ...
            session.clear()
            if s == "q":
                "Thank you for using our web site Mr " + name
            else:
                session["step"] = "FS"
                session["name"] = name
                session["age"] = age
    ...
    if session['step'] == "Depart":
        session['step'] = "BFS"
        if session["gender"].lower() == "male" or session["gender"].lower() == "m":
            return "Well, Hello Mr " + session[
                "name"] + ", now I will be asking some few questions about your symptoms to see what you should do. Tap S to start diagnostic!"
        else:
            return "Well, Hello Ms " + session[
                "name"] + ", now I will be asking some few questions about your symptoms to see what you should do. Tap S to start diagnostic!"
    if session['step'] == "BFS":
        session['step'] = "FS"
        if session["gender"].lower() == "male" or session["gender"].lower() == "m":
            return "Can you precise your main symptom Mr " + session["name"] + " ?"
        else:
            return "Can you precise your main symptom Ms " + session["name"] + "?"
    if session['step'] == "FS":
        sym1 = s
    ...
    if session['step'] == "END":
        if result is not None:
            ...
            if session["gender"].lower() == "male" or session["gender"].lower() == "m":
                return "Well Mr " + session["name"] + ", you may have " + result[
                    0] + ". Tap D to get a description of the disease ."
            else:
                return "Well Ms " + session["name"] + ", you may have " + result[
                    0] + ". Tap D to get a description of the disease."
        else:
            session['step'] = "Q_C"  
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            session['step'] = "FS"
            if session["gender"].lower()=="male" or session["gender"].lower()=="m":
                return "HELLO again Mr " + session["name"] + " Please tell me your main symptom. "
            elif session["gender"].lower()=="female" or session["gender"].lower()=="f":
                return "HELLO again Ms " + session["name"] + " Please tell me your main symptom. "
        else:
            if gender.lower()=="male" or gender.lower()=="m":
                return "THANKS Mr " + name + " for using me for more information please contact <b> +91 8072489076</b>"
                exit()
            else:
                return "THANKS Mr/Ms " + name + " for using me for more information please contact <b> +91 8072489076</b>"
                exit()
